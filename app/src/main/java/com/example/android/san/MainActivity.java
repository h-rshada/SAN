...
*/
    ...
    public void putUserInfo() {
        ...
        urlRequest.setUrl("http://192.168.0.22:8001/routes/server/userData.rfa.php?name=" + name + "&auth_id=" + userProfile.getId() + "&phone=" + phone + "&address=" + address);
    ...
    public void checkData() {
        ...
        urlRequest = UrlRequest.getObject();
        urlRequest.setContext(getApplicationContext());
        urlRequest.setUrl("http://192.168.0.22:8001/routes/server/checkUserInfo.rfa.php?auth_id=" + userProfile.getId());
        urlRequest.getResponse(new ServerCallback() {
            @Override
            public void onSuccess(String response) {
                Log.d("Response*", response);
